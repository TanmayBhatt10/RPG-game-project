...
pauseOverlay.innerHTML = `
  <div style="text-align: center; color: white;">
    <h1 style="font-size: 48px; margin-bottom: 20px; text-shadow: 0 0 20px cyan;">PAUSED</h1>
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && gameStarted && !state.gameOver) {
...
function animate(timestamp) {
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    // Remove dead enemies (but preserve boss with remaining lives)
    if (e.hp <= 0) {
      if (e.type === 'boss' && e.lives > 0) {
        // Boss still has lives, don't remove
        continue;
      }
      // Check if this is the boss being defeated at level 10
      if (e.type === 'boss' && state.level === 10) {
        bossDefeated = true;
  ...
  // Check for victory: boss defeated at level 10
  if (bossDefeated && state.level === 10) {
    state.gameOver = true;
